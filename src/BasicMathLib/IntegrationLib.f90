...
!the most used method for quadrature points and weights is Golub�CWelsch(GW) algorithm
...
implicit none
    ...
    public:: integrate
    !--
    public:: GaussGw    !based on the mkl symtridiag, more memory, more accurate
    ...
    end interface GaussHermite

	!---
	interface GaussGw
		procedure:: GaussGw_rec
		procedure:: GaussGw_rule
	end interface GaussGw
...
contains
    ...
    pure recursive real(rp) function &
	integrateAdaptiveSimpson_f1_rcs(f, lo, flo, up, fup, s, eps, max) result(integral)
    procedure(absf1)::          f
    ...
    real(rp)::                  mi,fmi,l,r
		...
		!15._rp comes from error analysis
        if(max==0.or.abs(l+r-s)<=15._rp*eps) then
    ...
    pure subroutine GaussGw_rec(alpha, beta, quadx, quadw)
    ...
    end subroutine GaussGw_rec

	!--
	pure subroutine GaussGw_rule(rule, quadx, quadw, par)
    character(*),intent(in)::			rule
    real(rp),dimension(:),intent(out):: quadx,quadw
	real(rp),dimension(:),optional,intent(in)::	par
    integer(ip)::                       n
	real(rp),dimension(size(quadx))::  	alpha,beta
    real(rp),dimension(size(quadx),size(quadx))::ev

        n = size(quadx)
		if(present(par)) then
			call recurCoef(rule, alpha, beta, par)
		else
			call recurCoef(rule, alpha, beta)
		endif
    ...
    character(len(rule))::                  r
        ...
        case('gausshermite_prob','gh_prob','hermite_prob')
            call GaussHermite(quadx,quadw,'prob')
    ...
    contains
        ...
        real(rp),dimension(ishft(n+1,-1)):: x0,dx,pm1,pm2,ppm1,ppm2,p,pp
            ...
            quadx = [-x0(ishft(n+1,-1) : 1 + pimod2(n) : -1), x0]
            !deriavtives
            quadw = [pp(ishft(n+1,-1) : 1 + pimod2(n):-1), pp]
        ...
        real(rp),dimension(ishft(n+1,-1)):: jk
            ...
            !give up, too much, waiting for some day
            call disableProgram
    ...
    integer(ip)::                           n
        ...
        !Normalise so that sum(w) = srpi
        quadw = (srpi/sum(quadw))*quadw

        !--for probability weight exp(-x^2/2)
        if(present(var)) then
            if(var=='prob') then
    ...
    contains
        ...
        integer(ip)::                       k
            ...
            if (pimod2(n)==1) then
                ...
                !find first root
                call alg2_Herm(H,n,x(pidb2(n)+1),ders(pidb2(n)+1))
            end if
        ...
        integer::                           i,k
        real(rp)::                          a,nu,p,tin
        real(rp),dimension(m)::             airyrts, x_init, x_init_airy, Tnk0
		real(rp),dimension(m)::             rhs, val, dval, dTnk0, tnk, x_init_sin
        real(rp),dimension(10)::            airyrts_exact
            ...
            ! correct first 10.
            airyrts(1:10) = airyrts_exact(1:10)
            ...
            x_init = real(sqrt(cmplx(nu + 2**(2._rp/3._rp)*airyrts*nu**(1._rp/3._rp) + &
                ...
                (15152._rp/3031875._rp*airyrts**5 + 1088._rp/121275._rp*airyrts**2)*&
                2**(1._rp/3._rp)*nu**(-7._rp/3._rp), rp)), rp)
            ...
            x_init_sin = sqrt(nu*tnk - (5._rp/(4._rp*(1._rp - tnk)**2) - &
                            1._rp/(1._rp-tnk) - 1._rp + 3._rp*a**2)/3._rp/nu)
            !Patch together
        ...
        real(rp),intent(in)::   x
            ...
            y = x**(2._rp/3._rp)*(1._rp + 5._rp/48._rp*x**(-2) - 5._rp/36._rp*x**(-4) + &
                    (77125._rp/82944._rp)*x**(-6) - 108056875._rp/6967296._rp*x**(-8) + &
                    162375596875._rp/334430208._rp*x**(-10))
	...
	!----------
	!https://github.com/chebfun/chebfun/blob/d6b2731d2437de3afe6c828332777652eff29304/jacpts.m
	!weight (1-x)^\alpha(1+x)^\beta
	pure subroutine GaussJacobi(alpha, beta, quadx, quadw)
	...
	real(rp)::							a, b
		...
		if(n==1) then
			quadx(1) = (b - a)/(a + b + 2._rp)
			quadw(1) = 2**(a + b + 1._rp)*betafc(a + 1._rp, b + 1._rp)
		endif

		!special case
		if(a==b) then
			if(a==0._rp) then
				!weight (1-x)^\alpha(1+x)^\beta = 1
				call GaussLegendre(quadx, quadw)
				return
			elseif(a==-0.5_rp) then
				!weight (1-x)^\alpha(1+x)^\beta = \frac{1}{\sqrt(1-x^2)}
				! call GaussChebyshev1(quadx, quadw)
			elseif(a==0.5_rp) then
				!weight (1-x)^\alpha(1+x)^\beta = \sqrt(1-x^2)
				! call GaussChebyshev2(quadx, quadw)
			endif
		endif
    ...
    integer(ip) function SparseGridSize(dim,level,quadratureRule) result(n)
    ...
    character(len(quadratureRule))::rule
        ...
        elseif(rule == 'gh' .or. rule == 'gausshermite' .or. &
		    ...
		    stop 'error: Integration/SparseGridSize get a mismathed quadratureRule!'
        endif
    ...
    !level -> in each dimension, a level determines the resolustion and collocation points increases with levelup
    ...
    contains
        ...
        real(rp),dimension(:),allocatable::     gw
            ...
            do ilvl=max(0, maxlvl+1-dim),maxlvl
                ...
                !sum(lvl_d) = ilvl and do recycle
                do; call compositionNext(ilvl,dim,lvl_d,more,h,t)
                    np_d = NpAtLevelClose(lvl_d)
					!tensor point number under this lvl_d(1:dim)
                    npTensor = product(np_d)
                    allocate(giTensor(dim, npTensor), gw(npTensor))

                    !giTensor map one-dimensional index to tensor multi-dimensional index
					!with index coordinate like (0, 0,...0)
                    call TensorGridIndexCfn(dim,np_d, giTensor)
